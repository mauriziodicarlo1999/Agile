	...
	***********************/
	...
	t.initCreateGrid = function(doAction){
		...
		jQuery('body').on('click', '#eg-add-column-advanced', function(){
			...
			jQuery('#eg-col-0').append('<td class="columns-adv-'+len+' columns-adv-rows columns-adv-head" style="text-align: center;position:relative;"></td>');
		...
		jQuery('body').on('click', '.esg-open-edit-dialog', function(){
			var cur_type = jQuery(this).attr('id').replace('esg-add-new-custom-', '');
...
/*			jQuery('#eg-lb-twitter').show();
	...
	**/
	...
	t.get_navigation_layout = function(){
		var elements = {pagination:{},left:{},right:{},filter:{},filter2:{},filter3:{},cart:{},sorting:{}}
	...
	***********************/
	...
	t.initSideButtons = function(i) {
		jQuery('.eg-side-buttons').each(function(i) {
			var bt = jQuery(this);
			punchgs.TweenLite.to(bt,0.3,{right:-55,delay:2+(i*0.1),ease:punchgs.Power3.easeOut});

			bt.hover(function() {
				punchgs.TweenLite.to(jQuery(this),0.3,{right:0});
			},function() {
				punchgs.TweenLite.to(jQuery(this),0.3,{right:-55});
			})
		})
...
******************************************************/
	...
	t.show3danim = function() {
		var orig = jQuery('.eg-editor-inside-wrapper');
				...
				punchgs.TweenLite.set(jQuery('#eg-3d-description'),0.5,{autoAlpha:0, delay:3});
